...
flow <- sf::st_read('data_tutorial1/Piave_Q21.5.shp')
...
hmu_map <- st_read('data_tutorial1/Piave_survey_hmu.shp')

## dissolve flow_wet or surveyed HMU polygons into 1 polygon for the wetted channel
# adjust max_hole_area if needed
channel_wet <- mesounitR::flow_to_wetchannel(hmu_map,max_hole_area = 20)

# alternatively: use flow_wet if discharge same as surveyed hmu
# channel_wet <- mesounitR::flow_to_wetchannel(flow_wet,max_hole_area = 20)
...
optimality_metrics <- mesounitR::compute_optimality_metrics(flow_wet,list_units,n_range,c_params,A_tot)

mesounitR::plot_segmentation_indicators(optimality_metrics)
...
n_range_toadd <- seq(20,40,by=2 )

list_units_old <- list_units
add_units_results <- mesounitR::add_units(list_units_old,supercells,n_range_toadd)
